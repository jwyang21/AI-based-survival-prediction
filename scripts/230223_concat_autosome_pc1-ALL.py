...
for cpg_type in all_cpg_types:
    for cohort in tcga_cohorts:
        ...
        for sample in cohort_samples:
            ...
            result_fname = os.path.join(cohort_data_dir, f'{sample}.pc1.raw-concat')
...
for cpg_type in all_cpg_types:
    ...
    for cohort in tcga_cohorts:
        cohort_data_dir = os.path.join(data_dir, cpg_type, cohort, 'pc1')
        all_concat_files = glob.glob(os.path.join(cohort_data_dir, f'*raw-concat*'))
    ...
    fname = f'concat_pc1_info_{cpg_type}.csv'
...
for cpg_type in all_cpg_types:
    for cohort in tcga_cohorts:
        ...
        for sample in cohort_samples:
            ...
            result_fname = os.path.join(cohort_data_dir, f'{sample}.pc1.standardized-concat')
            np.savez(result_fname, **globals()[f'{sample}'])
            #print(f'{sample}: {result_fname}.npz')
