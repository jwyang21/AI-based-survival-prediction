...
def parse_arguments():
    ...
    parser.add_argument('--data_version', type = str, help = 'input data version', required = True, default = None)
    parser.add_argument('--save_version', type = str, help = 'saving directory version', required = True, default = None)
...
if __name__ == '__main__':
    ...
    cohort_result_dir = os.path.join(args.result_dir, args.cpg_type, args.cohort, f'v{args.save_version}')
    if not os.path.exists(cohort_result_dir):
        os.makedirs(cohort_result_dir)
    print(f'cohort result dir: {cohort_result_dir}')
    
    cohort_data_dir = os.path.join(args.data_dir, args.cpg_type, args.cohort, f'v{args.data_version}')
    ...
    for target in all_survival_targets:
        ...
        for i in range(args.num_folds):
            ...
            hyperparams = {
                ...
                'batch_norm': True,
                'dropout': 0.4,
                'hidden_layers_sizes': [256, 128, 64], #number of nodes in each hidden layer #length of this array equals the total number of hidden layers inside the DeepSurv model.
                'learning_rate': 1e-05,
            ...
            if args.save_version == '1':
                hyperparams['hidden_layers_sizes'] = [500, 250, 250, 100, 50, 50]
                hyperparams['learning_rate'] = 1e-05
            elif args.save_version == '1.2':
                hyperparams['hidden_layers_sizes'] = [500, 250, 250, 100, 50, 50]
                hyperparams['learning_rate'] = 1e-04
            elif args.save_version == '2':
                hyperparams['hidden_layers_sizes'] = [256, 256, 128, 128, 25, 25]
                hyperparams['learning_rate'] = 1e-05
            elif args.save_version == '2.2':
                hyperparams['hidden_layers_sizes'] = [256, 256, 128, 128, 25, 25]
                hyperparams['learning_rate'] = 1e-04
            elif args.save_version == '3':
                hyperparams['hidden_layers_sizes'] = [2560, 1250, 500]
                hyperparams['learning_rate'] = 1e-05
            elif args.save_version == '3.2':
                hyperparams['hidden_layers_sizes'] = [2560, 1250, 500]
                hyperparams['learning_rate'] = 1e-04
            elif args.save_version == '4':
                hyperparams['hidden_layers_sizes'] = [250, 100, 25]
                hyperparams['learning_rate'] = 1e-05
            elif args.save_version == '4.2':
                hyperparams['hidden_layers_sizes'] = [250, 100, 25]
                hyperparams['learning_rate'] = 1e-04
            elif args.save_version == '5':
                hyperparams['hidden_layers_sizes'] = [128, 128]
                hyperparams['learning_rate'] = 1e-05
            elif args.save_version == '5.2':
                hyperparams['hidden_layers_sizes'] = [128, 128]
                hyperparams['learning_rate'] = 1e-04
            else:
                raise ValueError
            ...
            experiment_name = f'{args.cpg_type}-{cohort_type}-{current_item}-{args.save_version}-lr_{lr}'
            ...
            metrics_basename = f'{current_item}_lr_{lr}_metrics'
            ...
            model_name = os.path.join(cohort_result_dir, f'{current_item}_lr_{lr}_model')
            weight_name = os.path.join(cohort_result_dir, f'{current_item}_lr_{lr}_model_weights')
            ...
            plot_log_nll(metrics, cohort_result_dir, f'{current_item}_lr_{lr}_train_log_nll.png') #plots negative log likelihood and concordance index
            plot_log_c_index(metrics, cohort_result_dir, f'{current_item}_lr_{lr}_train_log_c-index.png') #plots negative log likelihood and concordance index
            result_df.loc[current_item]['train_valid_nll'] = os.path.join(cohort_result_dir, f'{current_item}_lr_{lr}_train_log_nll.png')
            result_df.loc[current_item]['train_valid_c_plot'] = os.path.join(cohort_result_dir, f'{current_item}_lr_{lr}_train_log_c-index.png')

            test_set_predicted_risk = model.predict_risk(test_data['x'])
            #print(test_set_predicted_risk.shape)

            test_df_with_risk_fname = f'{current_item}_lr_{lr}_test_df_with_risk.csv'
            ...
            valid_t, valid_t_pvalue, sig_t, sig_t_pvalue = survival_analysis_single_target(test_df_with_risk, target, cohort_result_dir, 2, f'LogRank_{current_item}_lr_{lr}.png', args.cohort)
            result_df.loc[current_item]['LogRank_p'] = valid_t_pvalue[0]
            result_df.loc[current_item]['LogRank_plot'] = os.path.join(cohort_result_dir, f'LogRank_{current_item}_lr_{lr}.png')
            del(model)
            result_pvalue = valid_t_pvalue[0]
            print(f'fold {i+1} {target} (lr={lr}) LogRank p-value: {result_pvalue}')
            plt.clf()
            
            current_result = result_df.loc[[current_item]].copy()
            current_result_fname = os.path.join(cohort_result_dir, f'{current_item}_lr_{lr}.csv')
            current_result.to_csv(current_result_fname)
            print(f'{current_item}: current_result_fname')
    result_df.to_csv(os.path.join(cohort_result_dir, f'all_versions_deepsurv_logrank_result_lr_{lr}.csv'))
    print(os.path.join(cohort_result_dir, f'all_versions_deepsurv_logrank_result_lr_{lr}.csv'))
